...
export const OrderCard = ({
  ...
  return (
    <div className="bg-white/[0.03] backdrop-blur-xl rounded-3xl p-8 border border-white/10 shadow-2xl group hover:bg-white/[0.05] transition-all duration-300">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <h3 className="text-xl font-bold text-white tracking-tight">Order #{order.id}</h3>
            {parsedAddress?.full_name && (
              <span className="px-2 py-0.5 rounded-md bg-white/5 text-[10px] font-bold text-gray-400 uppercase tracking-widest border border-white/5">
                {parsedAddress.full_name}
              </span>
            )}
          </div>
            ...
            {order.items.map((item, index) => (
              <li key={index} className="flex justify-between items-center group/item">
                <div className="flex items-center gap-3">
                  <span className="w-6 h-6 rounded-lg bg-white/5 flex items-center justify-center text-[10px] font-bold text-orange-500 border border-white/5">
                    {item.quantity}
                  </span>
                ...
                <span className="text-sm font-bold text-white">₹{(item.price || item.price_at_purchase) * item.quantity}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          {isVendor ? (
            <div className="flex gap-2 w-full sm:w-auto">
              {order.status === "PLACED" && (
                <>
                  <button
                    ...
                    disabled={disabled}
                    className="flex-1 sm:flex-none px-6 py-3 rounded-xl bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white font-bold text-sm shadow-lg shadow-orange-500/20 transition-all active:scale-95"
                  >
                    Accept
                  </button>
                  <button
                    ...
                    disabled={disabled}
                    className="flex-1 sm:flex-none px-6 py-3 rounded-xl bg-white/5 hover:bg-red-500/10 hover:text-red-500 disabled:opacity-50 text-gray-400 font-bold text-sm border border-white/10 transition-all active:scale-95"
                  >
                    Reject
                  </button>
                </>
              ...
              {order.status === "ACCEPTED" && (
                <button
                  ...
                  disabled={disabled}
                  className="w-full sm:w-auto px-8 py-3 rounded-xl bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white font-bold text-sm shadow-lg shadow-orange-500/20 transition-all active:scale-95"
                >
                  Assign Delivery
                </button>
              )}
            ...
            <Link
              ...
              className="w-full sm:w-auto px-8 py-3 rounded-xl bg-white/5 hover:bg-white/10 text-white font-bold text-sm border border-white/10 transition-all active:scale-95 text-center flex items-center justify-center gap-2 group/track"
            >
              Track Order
              <div className="w-1.5 h-1.5 rounded-full bg-orange-500 animate-pulse group-hover/track:scale-125 transition-transform" />
            </Link>
          )}
        </div>
      </div>
